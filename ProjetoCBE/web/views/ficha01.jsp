...
<%
    ...
    List<Ficha01> listaFichas;

    // Carrega as opções disponíveis no banco para os Dropdowns
    List<Integer> anosDisponiveis = ficha01Controller.getAnosExistentes();
    List<Integer> trimestresDisponiveis = ficha01Controller.getTrimestresExistentes();

    // Verificadores simples: O filtro está preenchido e não é "todos"?
    ...
    if (temAno && temTrimestre) {
        // CASO 1: Tem os DOIS filtros -> Busca composta
        int ano = Integer.parseInt(anoFiltro);
        int trim = Integer.parseInt(trimestreFiltro);
        listaFichas = ficha01Controller.getAllFichasByTrimestreAno(trim, ano);

    } else if (temAno) {
        // CASO 2: Tem APENAS Ano -> Busca só por ano
        int ano = Integer.parseInt(anoFiltro);
        listaFichas = ficha01Controller.getAllFichasByAno(ano);

    } else if (temTrimestre) {
        // CASO 3: Tem APENAS Trimestre -> Busca só por trimestre
        int trim = Integer.parseInt(trimestreFiltro);
        listaFichas = ficha01Controller.getAllFichasByTrimestre(trim);

    } else {
        // CASO 4: Nenhum filtro específico (ou ambos "todos") -> Busca tudo
        listaFichas = ficha01Controller.getAllFichas();
    }
    
    // Passa a lista processada para o contexto da página (para usar no c:forEach)
    ...
    <body>
        ...
        <div class="view-container">
                ...
                <div>
                    <c:choose>
                        <c:when test = "${comissao.startsWith('GER SOLUCOES')}">
                            <input type="button" class="btn btn-validar" value="Validar" data-ficha="ficha01" id="valida" title="É necessário ter o cargo de gerente para validar as informações">
                        </c:when>
                        <c:otherwise>
                    ...
                    <a href="../forms/ficha01.jsp"><input type="button" value="Adicionar" class="btn"></a>
                </div>
            </div>
            ...
            <div class="filtro-container" style="width: 95%; margin: 0 auto 20px auto; background-color: #f4f4f4; padding: 15px; border-radius: 8px; border: 1px solid #ddd;">
                <form action="ficha01.jsp" method="GET" style="display: flex; gap: 15px; align-items: center;">
                    ...
                    <button type="submit" class="btn" style="margin: 0; height: 40px;">Filtrar</button>
                    
                    <% if ((anoFiltro != null && !anoFiltro.equals("todos")) || (trimestreFiltro != null && !trimestreFiltro.equals("todos"))) { %>
                        <a href="ficha01.jsp" style="text-decoration: none; color: #b5131d; font-weight: bold; margin-left: 10px;">Limpar Filtro</a>
                    <% } %>
                </form>
            </div>
                    ...
                    <tr>
                        ...
                        <td class="opcoes-col">
                            <c:choose>
                                ...
                                <c:otherwise>
                                    <a href="../edits/ficha01.jsp?id=${ficha.getId()}" class="option-btn" title="Editar">
                                        <img class="option-btn-img" src="../resources/imgs/editar.png" alt="editar"/>
                                    </a>
                                    <button class="option-btn delete" title="Excluir" data-id="${ficha.getId()}" data-ficha="ficha01">
                                        <img class="option-btn-img" src="../resources/imgs/lixo.png" alt="alt"/>
                                    </button>
                                </c:otherwise>
                            </c:choose>
