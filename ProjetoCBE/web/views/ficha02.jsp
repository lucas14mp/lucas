...
<%
    ...
    List<Ficha02> listaFichas;

    // Carrega as opções disponíveis no banco para os Dropdowns
    List<Integer> anosDisponiveis = ficha02Controller.getAnosExistentes();
    List<Integer> trimestresDisponiveis = ficha02Controller.getTrimestresExistentes();
    ...
    if (temAno && temTrimestre) {
        ...
        listaFichas = ficha02Controller.getAllFichasByTrimestreAno(trim, ano);

    } else if (temAno) {
        // CASO 2: Tem APENAS Ano
        int ano = Integer.parseInt(anoFiltro);
        listaFichas = ficha02Controller.getAllFichasByAno(ano);

    } else if (temTrimestre) {
        // CASO 3: Tem APENAS Trimestre
        int trim = Integer.parseInt(trimestreFiltro);
        listaFichas = ficha02Controller.getAllFichasByTrimestre(trim);

    } else {
        // CASO 4: Busca tudo
        listaFichas = ficha02Controller.getAllFichas();
    }
    ...
    <body>
        ...
        <div class="view-container">
                ...
                <div>
                    <c:choose>
                        <c:when test = "${comissao.startsWith('GER SOLUCOES')}">
                            <input type="button" class="btn btn-validar" value="Validar" data-ficha="ficha02" id="valida" title="É necessário ter o cargo de gerente para validar as informações">
                        </c:when>
                        <c:otherwise>
                    ...
                    <a href="../forms/ficha02.jsp"><input type="button" value="Adicionar" class="btn"></a>
                </div>
            </div>
            ...
            <div class="filtro-container" style="width: 95%; margin: 0 auto 20px auto; background-color: #f4f4f4; padding: 15px; border-radius: 8px; border: 1px solid #ddd;">
                <form action="ficha02.jsp" method="GET" style="display: flex; gap: 15px; align-items: center;">
                    ...
                    <% if ((anoFiltro != null && !anoFiltro.equals("todos")) || (trimestreFiltro != null && !trimestreFiltro.equals("todos"))) { %>
                        <a href="ficha02.jsp" style="text-decoration: none; color: #b5131d; font-weight: bold; margin-left: 10px;">Limpar Filtro</a>
                    <% } %>
                </form>
            </div>
            <table class="table-lista-fichas">
                <tr>
                    ...
                    <th>Dividendos e outros rendimentos recebidos no período-base</th>
                    ...
                    <tr>
                        ...
                        <td class="opcoes-col">                           
                            <c:choose>
                                ...
                                <c:otherwise>
                                    <a href="../edits/ficha02.jsp?id=${ficha.getId()}" class="option-btn" title="Editar">
                                        <img class="option-btn-img" src="../resources/imgs/editar.png" alt="editar"/>
                                    </a>
                                    <button class="option-btn delete" title="Excluir" data-id="${ficha.getId()}" data-ficha="ficha02">
                                        <img class="option-btn-img" src="../resources/imgs/lixo.png" alt="alt"/>
                                    </button>
                                </c:otherwise>
                            </c:choose>
