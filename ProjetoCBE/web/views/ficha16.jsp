...
<%
    ...
    List<Ficha16> listaFichas;

    // Carrega as opções disponíveis
    List<Integer> anosDisponiveis = ficha16Controller.getAnosExistentes();
    List<Integer> trimestresDisponiveis = ficha16Controller.getTrimestresExistentes();
    ...
    if (temAno && temTrimestre) {
        ...
        listaFichas = ficha16Controller.getAllFichasByTrimestreAno(trim, ano);

    } else if (temAno) {
        // CASO 2: Tem APENAS Ano
        int ano = Integer.parseInt(anoFiltro);
        listaFichas = ficha16Controller.getAllFichasByAno(ano);

    } else if (temTrimestre) {
        // CASO 3: Tem APENAS Trimestre
        int trim = Integer.parseInt(trimestreFiltro);
        listaFichas = ficha16Controller.getAllFichasByTrimestre(trim);

    } else {
        // CASO 4: Busca tudo
        listaFichas = ficha16Controller.getAllFichas();
    }
    ...
    <body>
        ...
        <div class="view-container">
            <div class="topo-view">
                ...
                <div>
                    <c:choose>
                        <c:when test = "${nomeComissao.startsWith('GER') || chave == 'T1092713'}">
                            <input type="button" class="btn btn-validar" value="Validar" data-ficha="ficha16" id="valida" title="É necessário ter o cargo de gerente para validar as informações">
                        </c:when>
                        <c:otherwise>
                    ...
                    <a href="../forms/ficha16.jsp"><input type="button" value="Adicionar" class="btn"></a>
                </div>
            </div>
            ...
            <div class="filtro-container" style="width: 95%; margin: 0 auto 20px auto; background-color: #f4f4f4; padding: 15px; border-radius: 8px; border: 1px solid #ddd;">
                <form action="ficha16.jsp" method="GET" style="display: flex; gap: 15px; align-items: center;">
                    ...
                    <% if ((anoFiltro != null && !anoFiltro.equals("todos")) || (trimestreFiltro != null && !trimestreFiltro.equals("todos"))) { %>
                        <a href="ficha16.jsp" style="text-decoration: none; color: #b5131d; font-weight: bold; margin-left: 10px;">Limpar Filtro</a>
                    <% } %>
                </form>
            </div>
                    ...
                    <tr>
                        ...
                        <td class="opcoes-col">
                            <c:choose>
                                ...
                                <c:otherwise>
                                    <a href="../edits/ficha16.jsp?id=${ficha.getId()}" class="option-btn" title="Editar">
                                        <img class="option-btn-img" src="../resources/imgs/editar.png" alt="editar"/>
                                    </a>
                                    <button class="option-btn delete" title="Excluir" data-id="${ficha.getId()}" data-ficha="ficha16">
                                        <img class="option-btn-img" src="../resources/imgs/lixo.png" alt="alt"/>
                                    </button>
                                </c:otherwise>
                            </c:choose>
