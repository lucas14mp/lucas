...
<%@ page import="br.com.bb.cbe.controllers.Ficha12Controller" %>
<%@ page import="br.com.bb.cbe.Bean.Ficha12" %>
<%
    Ficha12Controller fichaController = new Ficha12Controller();
    int idFicha = Integer.parseInt(request.getParameter("id"));
    Ficha12 ficha = fichaController.getFichaById(idFicha);
...
<html>
    <head>
        ...
        <title>Atualização Ficha 12</title>
    </head>
    <body>
        ...
        <main class="main">
            <article class="article">
                ...
                <h2>Atualização Ficha 12 - Empréstimo intercompanhia</h2>
                <form action="<%=request.getContextPath()%>/ficha12" method="post" class="form">
                    ...
                    <select name="empresa" id="empresa" required>
                        ...
                        <c:forEach items="${empresaController.listarEmpresas()}" var="empresa">
                            <option value="${empresa.getId()}" ${empresa.getId() == ficha.getEmpresa().getId() ? 'selected' : ''}>${empresa.getNome()}</option>
                        </c:forEach>                            
                    </select>
                    ...
                    <select name="moeda" id="moeda" required>
                        ...
                        <c:forEach items="${moedaController.listarMoedas()}" var="moeda">
                            <option value="${moeda.getId()}" ${moeda.getId() == ficha.getMoeda().getId() ? 'selected' : ''}>${moeda.getNome()} | ${moeda.getSigla()}</option>
                        </c:forEach>
                    </select>
                    ...
                    <c:choose>
                        <c:when test="${ficha.getPrazoEmprestimo() == 'Até 12 meses'}">
                            <label>
                                <input type="radio" name="prazo" value="Até 12 meses" required checked>
                                Até 12 meses
                            </label>
                            <br>
                            <br>
                        ...
                        <c:otherwise>
                            ...
                            <br>
                            <label>
                                <input type="radio" name="prazo" value="Mais de 12 meses" required checked>
                                Mais de 12 meses
                            </label>
                        </c:otherwise>
                    </c:choose>
